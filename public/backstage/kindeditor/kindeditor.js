...
//系统表情路径(访问路径为http://域名/backstage/kindeditor/plugins/emoticons/twemoji/48.svg 打包后在nginx环境下，管理后台后端不能用这样的路径访问了，需要安装前台前端才能正常显示表情)
...
K.options = {
	...
	emoticonsPath: K.pageBasePath+'backstage/kindeditor/plugins/emoticons/twemoji/',
	...
	self.clickToolbar(name, function() {
		...
		function bindCellEvent(cell, j, num) {
			...
			cell.click(function(e) {
				...
				if(num <=total){
					//将域名和虚拟目录删除
					var relativePath = path.replace(K.pageBasePath, '');
					self.insertHtml('<img width="32px" height="32px" src="' + relativePath + num + '.svg" border="0" alt="" />').hideMenu().focus();
				}
				e.stop();
			});
		}
