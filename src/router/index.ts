...
const routes = [
    {path : '/admin/control/manage/index',component: () => import('@/views/index.vue'),
        children: [//嵌套一级子路由
            ...
            {path : '/admin/control/frontendApi/list',component : () => import('@/views/frontendModule/frontendApiList.vue'), name:'frontendApiList',meta: {index:'3-300300-1',title:'API列表',cacheNumber:'0'}},//前台API列表
            {path : '/admin/control/frontendApi/manage/add',component : () => import('@/views/frontendModule/addFrontendApi.vue'), name:'addFrontendApi',meta: {parent:'3-300300-1',title:'添加前台API'},
                beforeEnter: (to:any, from:any, next:any) => {
                    const store = useStore(pinia);
                    if(from.name == 'frontendApiList'){//如果来自前台API列表列表,则删除缓存
                        store.setCacheNumber()
                    }
                    next();
                }
            },//添加前台API
            {path : '/admin/control/frontendApi/manage/edit',component : () => import('@/views/frontendModule/editFrontendApi.vue'),name:'editFrontendApi',meta: {parent:'3-300300-1',title:'修改前台API'},
                beforeEnter: (to:any, from:any, next:any) => {
                    const store = useStore(pinia);
                    if(from.name == 'frontendApiList'){//如果来自前台API列表,则删除缓存
                         store.setCacheNumber()
                    }
                    next();
                }
            },//修改前台API
            {path : '/admin/control/frontendDocument/list',component : () => import('@/views/frontendModule/documentList.vue'), name:'frontendDocumentList',meta: {index:'3-300300-2',title:'前台文档列表',cacheNumber:'0'}},//前台文档列表
            {path : '/admin/control/section/manage/list',component : () => import('@/views/frontendModule/sectionList.vue'),name:'sectionList',meta: {index:'3-300300-3',title:'栏目列表',cacheNumber:'0'}},//栏目列表
            ...
            {path : '/admin/control/filterWord/manage/view',component : () => import('@/views/filterWord/viewFilterWord.vue'), name:'viewFilterWord',meta: {index:'5-500100-3',title:'敏感词',cacheNumber:'0'}},//敏感词 
...
export default createRouter({
    ...
    history: createWebHistory(
    // 部署时的基础路径
    //process.env.NODE_ENV === 'production' ? '/admin2/' : '/admin/'
    //'/admin2/'
  ),
