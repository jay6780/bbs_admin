        ...
        <div class="data-form label-width-blank systemSettingModule" >
            <el-form label-width="270px"  @submit.native.prevent>
                ...
                <el-form-item label="允许注册账号类型" :error="error.allowRegisterAccountType" v-show="state.activeTag == 10">
                    <el-select ref="allowRegisterAccountTypeRef"  v-model="state.allowRegisterAccountTypeCodeGroup" size="large" @remove-tag="processRemoveAllowRegisterAccountType" @click="loadAllowRegisterAccountType"  multiple :placeholder="state.select_allowRegisterAccountType_placeholder" >
                ...
                <el-form-item label="允许登录账号类型" :error="error.allowLoginAccountType" v-show="state.activeTag == 10">
                    <el-select ref="allowLoginAccountTypeRef"  v-model="state.allowLoginAccountTypeCodeGroup" size="large" @remove-tag="processRemoveAllowLoginAccountType" @click="loadAllowLoginAccountType"  multiple :placeholder="state.select_allowLoginAccountType_placeholder" >
                ...
                <el-form-item label="AI助手每分钟提交超过" :error="error.aiAssistant_submitQuantity" v-show="state.activeTag == 10">
                    <el-col :span="6"><el-input v-model.trim="state.aiAssistant_submitQuantity" disabled :required="true" maxlength="8" :clearable="true" show-word-limit></el-input></el-col>
                    <div class="form-help" >0为每次都出现验证码</div>
                </el-form-item>
                ...
                <el-form-item label="前台发表话题审核" :error="error.topic_review" v-show="state.activeTag == 10">
                    <el-radio-group v-model="state.topic_review" class="radio-group">
                        <el-radio :label="10">全部审核</el-radio>
                        <el-radio :label="30">特权会员免审核</el-radio>
                        <el-radio :label="50">无需审核</el-radio>
                    </el-radio-group>
                    <div class="form-help" >如果配置了风控功能，则触发风控’话题’会自动设为‘待审核’状态</div>
                </el-form-item>
                <el-form-item label="前台发表评论审核" :error="error.comment_review" v-show="state.activeTag == 10">
                    <el-radio-group v-model="state.comment_review" class="radio-group">
                        <el-radio :label="10">全部审核</el-radio>
                        <el-radio :label="30">特权会员免审核</el-radio>
                        <el-radio :label="50">无需审核</el-radio>
                    </el-radio-group>
                    <div class="form-help" >如果配置了风控功能，则触发风控’评论’会自动设为‘待审核’状态</div>
                </el-form-item>
                <el-form-item label="前台发表回复审核" :error="error.reply_review" v-show="state.activeTag == 10">
                    <el-radio-group v-model="state.reply_review" class="radio-group">
                        <el-radio :label="10">全部审核</el-radio>
                        <el-radio :label="30">特权会员免审核</el-radio>
                        <el-radio :label="50">无需审核</el-radio>
                    </el-radio-group>
                    <div class="form-help" >如果配置了风控功能，则触发风控’评论回复’会自动设为‘待审核’状态</div>
                </el-form-item>
                <el-form-item label="前台提交问题审核" :error="error.question_review" v-show="state.activeTag == 10">
                    <el-radio-group v-model="state.question_review" class="radio-group">
                        <el-radio :label="10">全部审核</el-radio>
                        <el-radio :label="30">特权会员免审核</el-radio>
                        <el-radio :label="50">无需审核</el-radio>
                    </el-radio-group>
                    <div class="form-help" >如果配置了风控功能，则触发风控‘问题‘会自动设为‘待审核’状态</div>
                </el-form-item>
                <el-form-item label="前台提交答案审核" :error="error.answer_review" v-show="state.activeTag == 10">
                    <el-radio-group v-model="state.answer_review" class="radio-group">
                        <el-radio :label="10">全部审核</el-radio>
                        <el-radio :label="30">特权会员免审核</el-radio>
                        <el-radio :label="50">无需审核</el-radio>
                    </el-radio-group>
                    <div class="form-help" >如果配置了风控功能，则触发风控‘答案‘会自动设为‘待审核’状态</div>
                </el-form-item>
                <el-form-item label="前台提交答案回复审核" :error="error.answerReply_review" v-show="state.activeTag == 10">
                    <el-radio-group v-model="state.answerReply_review" class="radio-group">
                        <el-radio :label="10">全部审核</el-radio>
                        <el-radio :label="30">特权会员免审核</el-radio>
                        <el-radio :label="50">无需审核</el-radio>
                    </el-radio-group>
                    <div class="form-help" >如果配置了风控功能，则触发风控‘答案回复‘会自动设为‘待审核’状态</div>
                </el-form-item>
                ...
                <el-form-item label="话题发起投票选项数量" :error="error.topicMaxVoteOptions" v-show="state.activeTag == 10">
                    <el-col :span="6"><el-input v-model.trim="state.topicMaxVoteOptions" :required="true" maxlength="8" :clearable="true" show-word-limit></el-input></el-col>
                    <div class="form-help" >空值为不限制；  0为不允许发起投票</div>
                </el-form-item>
                <el-form-item label="问答发起投票选项数量" :error="error.questionMaxVoteOptions" v-show="state.activeTag == 10">
                    <el-col :span="6"><el-input v-model.trim="state.questionMaxVoteOptions" :required="true" maxlength="8" :clearable="true" show-word-limit></el-input></el-col>
                    <div class="form-help" >空值为不限制；  0为不允许发起投票</div>
                </el-form-item>
                ...
                <el-form-item label="提及" v-show="state.activeTag == 20">
                    ...
                    <el-radio-group class="radioPosition"  v-model="state.topicEditorTagObject.mention">
                        <el-radio :label="true" disabled>打开</el-radio>
                        <el-radio :label="false" disabled>关闭</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="AI助手" v-show="state.activeTag == 20">
                    <span class="icon-ai" style="width: 16px;height: 16px;position: relative;top: -1px;margin-left: -9px;margin-right: 9px;"></span>
                    <el-radio-group class="radioPosition"  v-model="state.topicEditorTagObject.ai">
                        <el-radio :label="true" disabled>打开</el-radio>
                        <el-radio :label="false" disabled>关闭</el-radio>
                    </el-radio-group>
                </el-form-item>
                ...
                <el-form-item label="提及" v-show="state.activeTag == 30">
                    <span class="icon-mention" style="width: 16px;height: 16px;position: relative;top: -1px;margin-left: -9px;margin-right: 9px;"></span>
                    <el-radio-group class="radioPosition"  v-model="state.editorTagObject.mention">
                        <el-radio :label="true" disabled>打开</el-radio>
                        <el-radio :label="false" disabled>关闭</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="AI助手" v-show="state.activeTag == 30">
                    <span class="icon-ai" style="width: 16px;height: 16px;position: relative;top: -1px;margin-left: -9px;margin-right: 9px;"></span>
                    <el-radio-group class="radioPosition"  v-model="state.editorTagObject.ai">
                        <el-radio :label="true" disabled>打开</el-radio>
                        <el-radio :label="false" disabled>关闭</el-radio>
                    </el-radio-group>
                </el-form-item>
                ...
                <el-form-item label="提及" v-show="state.activeTag == 40">
                    <span class="icon-mention" style="width: 16px;height: 16px;position: relative;top: -1px;margin-left: -9px;margin-right: 9px;"></span>
                    <el-radio-group class="radioPosition"  v-model="state.questionEditorTagObject.mention">
                        <el-radio :label="true" disabled>打开</el-radio>
                        <el-radio :label="false" disabled>关闭</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="AI助手" v-show="state.activeTag == 40">
                    <span class="icon-ai" style="width: 16px;height: 16px;position: relative;top: -1px;margin-left: -9px;margin-right: 9px;"></span>
                    <el-radio-group class="radioPosition"  v-model="state.questionEditorTagObject.ai">
                        <el-radio :label="true" disabled>打开</el-radio>
                        <el-radio :label="false" disabled>关闭</el-radio>
                    </el-radio-group>
                </el-form-item>
                ...
                <el-form-item label="提及" v-show="state.activeTag == 50">
                    <span class="icon-mention" style="width: 16px;height: 16px;position: relative;top: -1px;margin-left: -9px;margin-right: 9px;"></span>
                    <el-radio-group class="radioPosition"  v-model="state.answerEditorTagObject.mention">
                        <el-radio :label="true" disabled>打开</el-radio>
                        <el-radio :label="false" disabled>关闭</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="AI助手" v-show="state.activeTag == 50">
                    <span class="icon-ai" style="width: 16px;height: 16px;position: relative;top: -1px;margin-left: -9px;margin-right: 9px;"></span>
                    <el-radio-group class="radioPosition"  v-model="state.answerEditorTagObject.ai">
                        <el-radio :label="true" disabled>打开</el-radio>
                        <el-radio :label="false" disabled>关闭</el-radio>
                    </el-radio-group>
                </el-form-item>
            ...
            <div class="selectSupportLanguageModule">
                <!-- 多语言切换 -->
                <el-dialog title="选择支持的语言" v-model="state.popup_languageSwitching" :draggable="true"  @close="closeSupportLanguage">
                    <div class="supportLanguageNavigation">
                        <div class="tab-content">
                            <div class="tab-pane">
                                <span :class="state.selectedLanguageSwitchingClass.get(key)" @click="selectedSupportLanguage(key,value)"  v-for="([key, value], index) in state.supportLanguage.entries()" >
                                    {{value}}
                                </span>
                            </div>
                        </div>
                ...
                <!-- 多语言表单扩展 -->
                <el-dialog title="选择支持的语言" v-model="state.popup_languageFormExtension" :draggable="true"  @close="closeLanguageFormExtension">
                    <div class="supportLanguageNavigation">
                        <div class="tab-content">
                            <div class="tab-pane">
                                <span :class="state.selectedLanguageFormExtensionClass.get(key)" @click="selectedLanguageFormExtension(key,value)"  v-for="([key, value], index) in state.supportLanguage.entries()" >
                                    {{value}}
                                </span>
                            </div>
                        </div>
...
<script lang="ts" setup>
    ...
    const languageFormExtensionRef = ref()
    ...
    const state = reactive({
        ...
        supportLanguage:new Map<string,string>(),//支持语言
        popup_languageSwitching:false,//是否显示多语言切换弹窗
        selectedLanguageSwitchingClass:new Map<string,string>(),//选中‘多语言表单扩展‘样式
        select_languageSwitching_placeholder: '请选择',
        languageSwitchingCodeGroup :[] as any,//多语言切换代码组
        languageSwitchingOptions: [] as Array<OptionsItem>,//Select 选择器多语言切换数据

        popup_languageFormExtension:false,//是否显示多语言表单扩展弹窗
        selectedLanguageFormExtensionClass:new Map<string,string>(),//选中‘多语言表单扩展‘样式
        select_languageFormExtension_placeholder: '请选择',
        languageFormExtensionCodeGroup :[] as any,//多语言表单扩展代码组
        languageFormExtensionOptions: [] as Array<OptionsItem>,//Select 选择器多语言表单扩展数据
    ...
    //多语言切换 -- 关闭支持语言弹出框
    const closeSupportLanguage = () => {
        state.popup_languageSwitching = false;

    }
    //多语言切换 -- 选中支持语言样式
    const setLanguageSwitchingClass = () => {
        for(let [code,name] of state.supportLanguage.entries()){
            state.selectedLanguageSwitchingClass.set(code,"supportLanguage-tag");
            if(state.languageSwitchingCodeGroup != null && state.languageSwitchingCodeGroup.length >0){
                for(let i = 0; i <state.languageSwitchingCodeGroup.length; i++){
                    let code = state.languageSwitchingCodeGroup[i];
                    state.selectedLanguageSwitchingClass.set(code,"supportLanguage-tag selected");
    ...
    //多语言切换 -- 选中支持语言
    const selectedSupportLanguage = (code:string,name:string) => {
       
        //判断是否重复选择,如果重复则取消选择
        if(state.languageSwitchingOptions != null && state.languageSwitchingOptions.length >0){
            for(var i=0; i<state.languageSwitchingOptions.length; i++){
                var selectedLanguageSwitchingOptions = state.languageSwitchingOptions[i];
                if(selectedLanguageSwitchingOptions.value == code){
                    //删除支持语言
                    deleteSupportLanguage(selectedLanguageSwitchingOptions.value);
                    setLanguageSwitchingClass();
                    return;
                }
            }
        ...
        state.languageSwitchingOptions.push(o);
        
        state.languageSwitchingCodeGroup.push(code);
        
        state.select_languageSwitching_placeholder = "";
        setLanguageSwitchingClass();
    }

    //多语言切换 -- 删除支持语言
    const deleteSupportLanguage = (code:string) => {
		if(state.languageSwitchingOptions != null && state.languageSwitchingOptions.length >0){
            for(let i=0; i<state.languageSwitchingOptions.length; i++){
                let selectedTag = state.languageSwitchingOptions[i];
                if(selectedTag.value == code){
                    state.languageSwitchingOptions.splice(i, 1);
                    
                    for(let j=0; j<state.languageSwitchingCodeGroup.length; j++){
                        if(state.languageSwitchingCodeGroup[j] == code){
                            state.languageSwitchingCodeGroup.splice(j, 1);
                            break;
                        }
                        
                    }
                    
                    if(state.languageSwitchingCodeGroup.length ==0){
                        state.select_languageSwitching_placeholder = "请选择";
                    }
                    return;
                }
            }
    ...
    //多语言切换 -- 处理删除支持语言
    const processRemoveLanguageSwitching = (val:any) => {
        if(state.languageSwitchingCodeGroup.length ==0){
    ...
    //多语言切换 -- 加载支持语言
    const loadLanguageSwitching = () => {
        state.popup_languageSwitching = true;
        languageSwitchingRef.value.blur();//使Select选择器失去焦点，并隐藏下拉框
        if(state.languageSwitchingCodeGroup.length ==0){
            state.languageSwitchingOptions.length = 0;//清空	
        }
        setLanguageSwitchingClass();
    }
    ...
    //多语言表单扩展 -- 关闭支持语言弹出框
    const closeLanguageFormExtension = () => {
        state.popup_languageFormExtension = false;

    }
    //设置多语言表单扩展 -- 选中支持语言样式
    const setLanguageFormExtensionClass = () => {
        for(let [code,name] of state.supportLanguage.entries()){
            state.selectedLanguageFormExtensionClass.set(code,"supportLanguage-tag");
            if(state.languageFormExtensionCodeGroup != null && state.languageFormExtensionCodeGroup.length >0){
                for(let i = 0; i <state.languageFormExtensionCodeGroup.length; i++){
                    let code = state.languageFormExtensionCodeGroup[i];
                    state.selectedLanguageFormExtensionClass.set(code,"supportLanguage-tag selected");
    ...
    //多语言表单扩展 -- 选中支持语言
    const selectedLanguageFormExtension = (code:string,name:string) => {
       
        //判断是否重复选择,如果重复则取消选择
        if(state.languageFormExtensionOptions != null && state.languageFormExtensionOptions.length >0){
            for(var i=0; i<state.languageFormExtensionOptions.length; i++){
                var selectedLanguageFormExtensionOptions = state.languageFormExtensionOptions[i];
                if(selectedLanguageFormExtensionOptions.value == code){
                    //删除支持语言
                    deleteLanguageFormExtension(selectedLanguageFormExtensionOptions.value);
                    setLanguageFormExtensionClass();
                    return;
                }
            }
        ...
        state.languageFormExtensionOptions.push(o);
        
        state.languageFormExtensionCodeGroup.push(code);
        
        state.select_languageFormExtension_placeholder = "";
        setLanguageFormExtensionClass();
    }

    //多语言表单扩展 -- 删除支持语言
    const deleteLanguageFormExtension = (code:string) => {
		if(state.languageFormExtensionOptions != null && state.languageFormExtensionOptions.length >0){
            for(let i=0; i<state.languageFormExtensionOptions.length; i++){
                let selectedTag = state.languageFormExtensionOptions[i];
                if(selectedTag.value == code){
                    state.languageFormExtensionOptions.splice(i, 1);
                    
                    for(let j=0; j<state.languageFormExtensionCodeGroup.length; j++){
                        if(state.languageFormExtensionCodeGroup[j] == code){
                            state.languageFormExtensionCodeGroup.splice(j, 1);
                            break;
                        }
                        
                    }
                    
                    if(state.languageFormExtensionCodeGroup.length ==0){
                        state.select_languageFormExtension_placeholder = "请选择";
                    }
                    return;
                }
            }
    ...
    //多语言表单扩展 -- 处理删除支持语言
    const processRemoveLanguageFormExtension = (val:any) => {
        if(state.languageFormExtensionCodeGroup.length ==0){
    ...
    //多语言表单扩展 -- 加载支持语言
    const loadLanguageFormExtension = () => {
        state.popup_languageFormExtension = true;
        languageFormExtensionRef.value.blur();//使Select选择器失去焦点，并隐藏下拉框
        if(state.languageFormExtensionCodeGroup.length ==0){
            state.languageFormExtensionOptions.length = 0;//清空	
        }
        setLanguageFormExtensionClass();
    }
                ...
                if(result){
                    if(result.code === 200){//成功
			    		...
			    		for(let key in mapData){
			    			...
			    			}else if(key == "supportLanguage"){//支持语言
                                state.supportLanguage.clear();
                                for (let code in mapData[key]) {
                                    state.supportLanguage.set(code,mapData[key][code])
                                }
			    			...
			    			if(systemSetting != null){
                                ...
                                state.languageSwitchingCodeGroup.length =0;
                                state.languageSwitchingOptions.length =0;
                                if(systemSetting.languageSwitchingCodeList != null && systemSetting.languageSwitchingCodeList.length >0){
			    					A:for(let i = 0; i <systemSetting.languageSwitchingCodeList.length; i++){
                                        let languageSwitchingCode = systemSetting.languageSwitchingCodeList[i];
                                        
                                        for(let [code,name] of state.supportLanguage.entries()){
                                            if(code == languageSwitchingCode){
                                                state.languageSwitchingCodeGroup.push(languageSwitchingCode);
                                                let obj:any =new Object();
                                                obj.value = languageSwitchingCode;
                                                obj.label = name;
                                                state.languageSwitchingOptions.push(obj);
                                                continue A;
                                            }
                                        }
                                ...
                                state.languageFormExtensionCodeGroup.length =0;
                                state.languageFormExtensionOptions.length =0;
                                if(systemSetting.languageFormExtensionCodeList != null && systemSetting.languageFormExtensionCodeList.length >0){
			    					A:for(let i = 0; i <systemSetting.languageFormExtensionCodeList.length; i++){
                                        let languageFormExtensionCode = systemSetting.languageFormExtensionCodeList[i];
                                        
                                        for(let [code,name] of state.supportLanguage.entries()){
                                            if(code == languageFormExtensionCode){
                                                state.languageFormExtensionCodeGroup.push(languageFormExtensionCode);
                                                let obj:any =new Object();
                                                obj.value = languageFormExtensionCode;
                                                obj.label = name;
                                                state.languageFormExtensionOptions.push(obj);
                                                continue A;
                                            }
                                        }
                                ...
                                if(systemSetting.topicMaxVoteOptions != null){
			    					state.topicMaxVoteOptions = systemSetting.topicMaxVoteOptions;
			    				}
                                if(systemSetting.questionMaxVoteOptions != null){
			    					state.questionMaxVoteOptions = systemSetting.questionMaxVoteOptions;
			    				}
    ...
    const submitForm = () => {
        ...
        if(state.languageSwitchingCodeGroup != null && state.languageSwitchingCodeGroup.length >0){
            for(let i = 0; i <state.languageSwitchingCodeGroup.length; i++){
                let code = state.languageSwitchingCodeGroup[i];
                formData.append('languageSwitchingCodeList', code); 
            }
        }
        if(state.languageFormExtensionCodeGroup != null && state.languageFormExtensionCodeGroup.length >0){
            for(let i = 0; i <state.languageFormExtensionCodeGroup.length; i++){
                let code = state.languageFormExtensionCodeGroup[i];
                formData.append('languageFormExtensionCodeList', code); 
            }
        }
        ...
        if(state.topicMaxVoteOptions != null){
            formData.append('topicMaxVoteOptions', state.topicMaxVoteOptions);
        }
        if(state.questionMaxVoteOptions != null){
            formData.append('questionMaxVoteOptions', state.questionMaxVoteOptions);
        }
        ...
        if(state.topicEditorTagObject.ai != null){
            formData.append('topicEditorTagObject.ai', state.topicEditorTagObject.ai.toString());
        }
        ...
        if(state.editorTagObject.ai != null){
            formData.append('editorTagObject.ai', state.editorTagObject.ai.toString());
        }
        ...
        if(state.questionEditorTagObject.ai != null){
            formData.append('questionEditorTagObject.ai', state.questionEditorTagObject.ai.toString());
        }
        ...
        if(state.answerEditorTagObject.ai != null){
            formData.append('answerEditorTagObject.ai', state.answerEditorTagObject.ai.toString());
        }
...
/** 选择支持语言 **/
.selectSupportLanguageModule {
    background: #fff;
    .supportLanguageNavigation{
        font-size: 14px;
        .tab-content{
