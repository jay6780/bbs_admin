...
<script lang="ts" setup>
    import { type ComponentInternalInstance, getCurrentInstance, onMounted, reactive, onUnmounted} from 'vue';
        ...
        .then((response: AxiosResponse) => {
            if(response){
                ...
                if(result){
					...
					//服务器时间戳
					let serverDate = parseInt(result.currentTime)+time;
					
					
                    state.clock = dayjs(serverDate).utcOffset(result.timezoneOffset);
                }
            }
        })
        .catch((error: any) =>{
