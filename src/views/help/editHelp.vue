        ...
        <div class="data-form" >
            <el-form label-width="auto"  @submit.native.prevent>
                ...
                <el-form-item label="内容" :required="true" :error="error.content">
                    <div v-show="!state.isEditHelpMarkdown" style="width: 100%;">
                        <textarea ref="contentRef" :editorId="'editHelp'"  style="width:100%;height:400px;visibility:hidden;"></textarea>
                    </div>
                    <div v-if="state.isEditHelpMarkdown" style="width: 100%;">
                        <Editor :editorId="'editHelp'" :value="state.editHelpMarkdownContent" :plugins="state.editHelpEditorPlugin" :locale="zhHans" :sanitize="editHelpSanitize" placeholder="请输入内容..." @change="handleEditHelpMarkdownChange"/>
                    </div>
...
<script lang="ts" setup>
    ...
    const state = reactive({
        ...
        helpTypeId :'',//分类Id
        content :'',
        editHelpAvailableTag:[] as Array<string>,//修改帮助富文本编辑器允许使用的标签
        editHelpMarkdownContent:'',//markdown修改帮助内容
        isEditHelpMarkdown:false,//是否使用markdown帮助编辑器
        ...
        helpType_form:false,//是否显示帮助标签表单
        navigation: '',
        ...
        editHelpEditorPlugin:[] as BytemdPlugin[],//修改帮助Markdown编辑器插件
    });
    const error = reactive({
     ...
     //处理Markdown
     const handleEditHelpMarkdownChange = (value: string) => {
        state.editHelpMarkdownContent = value;
    }

    //白名单
    const editHelpSanitize = (schema: any) => {
        schema = sanitize(schema);
        
        if(state.editHelpAvailableTag?.indexOf('embedVideo') != -1){//嵌入视频
    ...
    //处理切换到富文本编辑器
    const handleEditHelpToggleRichtextEditor = (editorId: string) => {
        state.isEditHelpMarkdown= false;
        nextTick(()=>{
            if (Object.keys(state.editHelpEditorCreateParameObject).length != 0) {//不等于空
                ...
                state.editHelpEditor = createEditor(
                    ...
                    ()=>{
                        handleEditHelpToggleMarkdownEditor(editorId);
                    },
                    ()=>{
                    ...
                    ()=>{
                        //AI弹窗
                        aiPopUp(proxy,appContext,editorId,state.editHelpEditorCreateParameObject.uploadPath,'file',store_fileStorageSystem.value, state.isEditHelpMarkdown,state.editHelpEditor,undefined)
                    }
                );
                state.editHelpEditor.html(state.editHelpEditorCreateParameObject.html);//切换时设置富文本内容
            }
        })
    ...
    //处理切换到Markdown编辑器
    const handleEditHelpToggleMarkdownEditor = (editorId: string) => {
        
        if (Object.keys(state.editHelpEditor).length != 0) {//不等于空
            state.editHelpEditorCreateParameObject.html = state.editHelpEditor.html();//切换时保存富文本内容
			state.editHelpEditor.remove();
            state.editHelpEditor = {};
		}
        state.isEditHelpMarkdown = true;
    }
            ...
            if(response){
                ...
                if(result){
                    if(result.code === 200){//成功
                        let help = result.data;
			    		state.name = help.name;
			    		state.helpTypeId = help.helpTypeId;
                 
                        if(help.isMarkdown != null && help.isMarkdown == true){
                            state.isEditHelpMarkdown = true;
                            state.editHelpMarkdownContent = help.markdownContent;
                        }else{
                            state.isEditHelpMarkdown = false;
		    			...
		    			let availableTag = ['source', '|', 'preview', 'code', 
		    				        ...
		    				        'italic', 'underline', 'strikethrough', 'lineheight', 'removeformat', '|', 'image', 'multiimage',
		    				         ...
		    				         'link', 'unlink'];
                        state.editHelpAvailableTag = availableTag;//修改话题富文本编辑器允许使用的标签

                        let uploadPath = "control/help/manage?method=upload&userName="+state.userName+"&isStaff=true";
                        

		    		
                        if(!state.isEditHelpMarkdown){
                            //创建富文本编辑器
                            state.editHelpEditor = createEditor(contentRef.value, availableTag, uploadPath, null,store_fileStorageSystem.value,(editorId: string)=>{
                                handleEditHelpToggleMarkdownEditor(editorId);
                            },
                            (editorId)=>{
                                ...
                                //AI弹窗
                                aiPopUp(proxy,appContext,editorId,uploadPath,'file',store_fileStorageSystem.value,false,state.editHelpEditor,undefined)
                            });
                        }
    ...
    const submitForm = () => {
        ...
        if(state.isEditHelpMarkdown){
            formData.append('isMarkdown', state.isEditHelpMarkdown.toString());
            formData.append('markdownContent', state.editHelpMarkdownContent);
        }else{
        ...
        if (Object.keys(state.editHelpEditorCreateParameObject).length != 0) {//不等于空
            if(!state.isEditHelpMarkdown){
                ...
                state.editHelpEditor = createEditor(
                    ...
                    store_fileStorageSystem.value,
                    ()=>{
                        handleEditHelpToggleMarkdownEditor(state.editHelpEditorCreateParameObject.editorId);
                    },
                    ()=>{
                    ...
                    ()=>{
                        //AI弹窗
                        aiPopUp(proxy,appContext,"",state.editHelpEditorCreateParameObject.uploadPath,'file',store_fileStorageSystem.value,false,state.editHelpEditor,undefined)
                    }
                );
            }
		}
