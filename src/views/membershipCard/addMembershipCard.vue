...
<script lang="ts" setup>
    import { type ComponentInternalInstance, getCurrentInstance, onActivated, onDeactivated, onMounted, reactive, ref } from 'vue';
                ...
                if(result){
                    if(result.code === 200){//成功
			    		...
			    		if (Object.keys(state.introductionEditorCreateParameObject).length === 0) {//等于空
                            //创建富文本编辑器
                            state.introductionEditor = createEditor(introductionRef.value, availableTag, uploadPath, null,store_fileStorageSystem.value,(editorId: string)=>{},(editorId: string)=>{},
                                (editorId: string)=>{
                                    //AI弹窗
                                    aiPopUp(proxy,appContext,editorId,uploadPath,'file',store_fileStorageSystem.value,false,state.introductionEditor,undefined)
                                }
                            );
            ...
            if(response){
			    ...
			    if(result){
			    	if(result.code === 200){//成功
			    		...
			    		//处理错误信息
                        processErrorInfo_number(result.data as Map<string,string> , error,()=>{});
    ...
    onActivated (()=>{
        if (Object.keys(state.introductionEditorCreateParameObject).length != 0) {//不等于空
			...
			state.introductionEditor = createEditor(
                ...
                ()=>{
                    //AI弹窗
                    aiPopUp(proxy,appContext,"",state.introductionEditorCreateParameObject.uploadPath,'file',store_fileStorageSystem.value,false,state.introductionEditor,undefined)
                }
			);
		}
    })
