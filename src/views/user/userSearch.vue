            ...
            <el-form :inline="true" @keyup.enter.native="submitForm">
                ...
                <el-form-item :error="error.keyword" style="width: 280px;" v-if="state.searchType ==1">
                    <el-tooltip :content="'请输入'+state.userTypeGroup[state.userTypeCode]" placement="top">
                        <el-input v-model.trim="state.keyword" maxlength="100" style="width: 100%;"  :placeholder="state.userTypeGroup[state.userTypeCode]" clearable></el-input>
                    </el-tooltip>
                </el-form-item>
                <el-form-item :error="error.start_point" style="width: 180px;" v-if="state.searchType ==2">
                    <el-tooltip content="不限制请留空" placement="top">
                        <el-input v-model.trim="state.start_point" maxlength="30" placeholder="起始积分" clearable></el-input>
                    </el-tooltip>
                </el-form-item>
                <el-form-item :error="error.end_point" style="width: 180px;"  v-if="state.searchType ==2">
                    <el-tooltip content="不限制请留空" placement="top">
                        <el-input v-model.trim="state.end_point" maxlength="30" placeholder="结束积分" clearable></el-input>
                    </el-tooltip>
                </el-form-item>
                ...
                <template v-for="(userCustom,index) in state.userCustomList" v-if="state.searchType ==2">
                    <!--单选按钮 -->
                    <el-tooltip :content="userCustom.tip" placement="top" v-if="userCustom.chooseType ==2" :disabled="userCustom.tip == null || userCustom.tip == ''">
                        <el-form-item :error="error.userCustom.get('userCustom_'+userCustom.id)" >
                            <el-select v-model="state.userBoundField[index]" size="large" class="custom-select-width"  :placeholder="userCustom.name" clearable>
                                <el-option v-for="(value, key)  in userCustom.itemValue" :label="value" :value="key" ></el-option>
                            </el-select>
                        </el-form-item>
                    </el-tooltip>
                    <!--多选按钮 -->
                    <el-tooltip :content="userCustom.tip" placement="top" v-if="userCustom.chooseType ==3" :disabled="userCustom.tip == null || userCustom.tip == ''">
                        <el-form-item :error="error.userCustom.get('userCustom_'+userCustom.id)" ><!--multiple collapse-tags collapse-tags-tooltip -->
                            <el-select v-model="state.userBoundField[index]" size="large" class="custom-select-width"  :placeholder="userCustom.name"  clearable>
                                <el-option v-for="(value, key)  in userCustom.itemValue" :label="value" :value="key" ></el-option>
                            </el-select>
                        </el-form-item>
                    ...
                    <!--下拉列表 -->
                    <el-tooltip :content="userCustom.tip" placement="top" v-if="userCustom.chooseType ==4" :disabled="userCustom.tip == null || userCustom.tip == ''">
                        <el-form-item :error="error.userCustom.get('userCustom_'+userCustom.id)" ><!--:multiple="userCustom.multiple" collapse-tags collapse-tags-tooltip -->
                            <el-select v-model="state.userBoundField[index]" size="large" class="custom-select-width"  :placeholder="userCustom.name" clearable>
                                <el-option v-for="(value, key)  in userCustom.itemValue" :label="value" :value="key" ></el-option>
                            </el-select>
                        </el-form-item>
...
<script lang="ts" setup>
    ...
    import type { ExtensionFormValue, UserCustom } from '@/types';
    ...
    const state = reactive({
        ...
        extensionFormValue : {} as ExtensionFormValue,//扩展表单值
    ...
    const queryUserList = () => {
        ...
        proxy?.$axios({
            ...
            params: {
                ...
                ...state.extensionFormValue,
            },
            paramsSerializer: {
                indexes: null, //生成的查询参数无括号  /myAction?storeIds=1&storeIds=2&storeIds=3
            },
    ...
    const boundField = (userCustom:UserCustom) => {
        if (userCustom.chooseType == 2) { //单选框
            let checked = "";
    ...
    const submitForm = () => {
       ...
       // let extensionGroup = new Map();
       // let extensionValueGroup = new Object() as any;
        ...
        if (state.userCustomList != null && state.userCustomList.length > 0) {
            for (let i = 0; i < state.userCustomList.length; i++) {
                ...
                if (userCustom.chooseType == 2) { //单选框
                    ...
                    Object.assign(state.extensionFormValue, {['userCustom_'+ userCustom.id] : fieldValue});
                } else if (userCustom.chooseType == 3) { //多选框
                    //for (var value in fieldValue) {
                       // Object.assign(state.extensionFormValue, {['userCustom_'+ userCustom.id] : fieldValue[value]});
    ...
    onMounted(() => {
        ...
        for (let property in router.currentRoute.value.query) {
            if(property.startsWith("userCustom_")){
                Object.assign(state.extensionFormValue, {[property] : router.currentRoute.value.query[property]});
            }
