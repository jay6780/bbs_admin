...
<template>
    <div class="main">
        <div class="navbar">
            ...
            <el-button type="primary" plain v-if="verifyPermissionMenu('/control/penaltyUserInfo/riskControlPenaltyList*','get')" @click="router.push({path: '/admin/control/penaltyUserInfo/riskControlPenaltyList', query:{ id : route.query.id,userName : encodeURIComponent(state.user.userName)}})">风控处罚</el-button>
            <el-button type="primary" plain v-if="verifyPermissionMenu('/control/vote/userVoteTheme/list*','get')" @click="router.push({path: '/admin/control/vote/userVoteTheme/list', query:{ id : route.query.id,userName : encodeURIComponent(state.user.userName),beforeUrl:(route.query.beforeUrl != undefined ? route.query.beforeUrl:'')}})">发起投票</el-button>
            <el-button type="primary" plain v-if="verifyPermissionMenu('/control/vote/userVoteRecord/list*','get')" @click="router.push({path: '/admin/control/vote/userVoteRecord/list', query:{ id : route.query.id,userName : encodeURIComponent(state.user.userName),beforeUrl:(route.query.beforeUrl != undefined ? route.query.beforeUrl:'')}})">参与投票</el-button>
        ...
        <div class="data-view" >
            ...
            <el-row :gutter="10" type="flex" v-if="state.penaltyUserInfoList && state.penaltyUserInfoList.length >0" style="margin-top: 24px;">
                <el-col :span="4"><div class="name">风控处罚前10条记录：</div></el-col>
                <el-col :span="20" class="userModule">
                    <el-table :data="state.penaltyUserInfoList" tooltip-effect="dark" :row-class-name="tableRowClassName" stripe :style="{ width: '100%' }" empty-text="没有记录">
                        <el-table-column label="风控名称" align="center">
                            <template #default="scope">
                            
                                <el-link href="javascript:void(0);" class="link" @click="store.setCacheNumber(); router.push({path: '/admin/control/riskControlModel/manage/edit', query:{ riskControlModelId : scope.row.riskControlModelId,beforeUrl:(route.query.beforeUrl != undefined ? route.query.beforeUrl:'')}})">{{scope.row.riskControlModelName}}</el-link>
                                
                            </template>
                        </el-table-column>
                        <el-table-column prop="createTime" label="封禁起始时间" align="center" width="170"></el-table-column>
                        <el-table-column prop="banEndTime" label="封禁结束时间" align="center" width="170"></el-table-column>
                        <el-table-column label="操作" align="center" fixed="right" width="230">
                            <template #default="scope">
                                <div class="button-group-wrapper">
                                    <el-button-group>
                                        <el-button type="primary" :disabled="!verifyPermissionMenu('/control/penaltyUserInfo/manage?method=unblock&*','post')"  @click="unblock($event,scope.row)">解除封禁</el-button>
                                        <el-button type="primary" :disabled="!verifyPermissionMenu('/control/penaltyUserInfo/manage?method=view&*','get')" @click="router.push({path: '/admin/control/penaltyUserInfo/manage/view', query:{ penaltyUserInfoId : scope.row.id, page:state.currentpage}})">查看</el-button>
                                        <el-button type="primary" :disabled="!verifyPermissionMenu('/control/penaltyUserInfo/manage?method=delete&*','post')" @click="deletePenaltyUserInfo($event,scope.row)">删除</el-button>
                                    </el-button-group>
                                </div>
                            </template>
...
<script lang="ts" setup>
    ...
    import { ElMessage, ElMessageBox, ElTable, type UploadInstance, type UploadUserFile } from 'element-plus';
    import { processErrorInfo, updateURLParameter,verifyPermissionMenu } from '@/utils/tool';
    import type { SourceUrl, User, UserRole, UserCustom, PenaltyUserInfo } from '@/types';
    ...
    const state = reactive({
        ...
        penaltyUserInfoList:[] as Array<PenaltyUserInfo>,
    ...
    //隐藏行  {row,rowIndex}: {row: any,rowIndex: number}
                ...
                if(result){
                    if(result.code === 200){//成功
			    		...
			    		for(let key in mapData){
			    			...
			    			}else if(key == "penaltyUserInfoList"){
			    				state.penaltyUserInfoList = mapData[key];
			    			}else if(key == "user"){
    ...
    //解除封禁
    const unblock = (event:any,row:any) => {
        //强制失去焦点
        ...
        ElMessageBox.confirm('此操作将该项解除封禁, 是否继续?',{
            // type: 'warning',
            cancelButtonText: "取消",
            confirmButtonText: '确定'
        })
        .then(() => {
            ...
            let formData = new FormData();
		    formData.append('penaltyUserInfoId', row.id);
        

            proxy?.$axios({
                url: '/control/penaltyUserInfo/manage?method=unblock&a=a',//a=a参数的作用是仅增加连接符&
                method: 'post',
                data: formData
            })
            .then((response: AxiosResponse) => {
     ...
     //删除处罚用户信息
     const deletePenaltyUserInfo = (event:any,row:any) => {
        //强制失去焦点
        let target = event.target;
        // 根据button组件内容 里面包括一个span标签，如果设置icon，则还包括一个i标签，其他情况请自行观察。
        // 所以，在我们点击到button组件上的文字也就是span标签上时，直接执行e.target.blur()不会生效，所以要加一层判断。
        ...
        ElMessageBox.confirm('此操作将删除该处罚用户信息, 是否继续?',{
            // type: 'warning',
            cancelButtonText: "取消",
            confirmButtonText: '确定'
        })
        .then(() => {
            ...
            proxy?.$axios({
                url: '/control/penaltyUserInfo/manage?method=delete&a=a',//a=a参数的作用是仅增加连接符&
                method: 'post',
                data: formData
            })
            .then((response: AxiosResponse) => {
