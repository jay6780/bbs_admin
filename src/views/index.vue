            ...
            <el-aside :class="state.isLeftNavigationMenuCollapse ? 'aside-shrink' :'aside-expand'" >
                ...
                <el-scrollbar>
                    ...
                    <el-menu v-if="state.topNavigationActiveIndex =='3' || state.mini" :default-active="state.leftNavigationMenuActive" :collapse="state.isLeftNavigationMenuCollapse" text-color="#606266" class="navigation-menu-left" @open="leftNavigationMenuOpen" @close="leftNavigationMenuClose"  @select="leftNavigationMenuSelect" :default-openeds="state.leftNavigationMenuOpeneds">
						...
						<el-sub-menu index="3-300300" v-show="letfNavigationMenuVerifyPermission('3-300300')">
                            <template #title>
								<el-icon class="icon"><Document /></el-icon>
                                <span>前台模块管理</span>
                            </template>
                            <el-menu-item index="3-300300-1" v-if="navigationSubMenuVerifyPermission('3-300300-1')">API列表</el-menu-item>
							<el-menu-item index="3-300300-2" v-if="navigationSubMenuVerifyPermission('3-300300-2')">前台文档列表</el-menu-item>
							<el-menu-item index="3-300300-3" v-if="navigationSubMenuVerifyPermission('3-300300-3')">站点栏目</el-menu-item>
                        </el-sub-menu>
                    </el-menu>
								...
								<div class="homeMenu">  
									...
									<div class="nav-list-user">
										<el-dropdown size="large">
											...
											<template #dropdown>
												<el-dropdown-menu>
													<el-dropdown-item :icon="Tools" v-if="state.sysUsers.issys" @click="jumpPage('2-200300-1',()=>{router.push({path : '/admin/control/staff/manage/editStaff',query:{userId : state.sysUsers.userId}})})">个人设置</el-dropdown-item>
													<el-dropdown-item :icon="Tools" :disabled="!verifyPermissionMenu('/control/staff/manage?method=editSelfInfo*','get')" v-if="!state.sysUsers.issys" @click="jumpPage('2-200300-1',()=>{router.push({path : '/admin/control/staff/manage/editSelfInfo'})})">个人设置</el-dropdown-item>
                ...
                <el-main>
                    ...
                    <router-view v-slot="{ Component }" >
                        <keep-alive :include="cacheComponents" :max="200">
							<component :is="Component" :key="store.getCacheKey" v-if="state.isRouterAlive" @onJumpPage="jumpPage"/> 
                        </keep-alive>
                    </router-view>
                </el-main>
            </el-container>
...
<script lang="ts" setup>
    import { type ComponentInternalInstance, computed, getCurrentInstance, nextTick, onMounted, reactive, ref, watch, watchEffect } from 'vue';
	...
	import { type MenuLinkTag, type SysUsers } from '@/types';
	...
	//跳转网页
	const jumpPage = (index:string,callback: () => void) => {
		createWindow(index);//"2-200300-1"
		callback();
    }
	...
	const navigationMenuIndexMapping = new Map<string,string>([
		...
		["3-300300-1", "/control/frontendApi/list*"], //API列表
		["3-300300-2", "/control/frontendDocument/list*"], //前台文档列表
		["3-300300-3", "/control/section/manage?method=list*"], //站点栏目
		...
		["5-500100-7", "/control/emailSetting/manage/edit*"], //邮箱设置
