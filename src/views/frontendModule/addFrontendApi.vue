                                        ...
                                        <template #file="{ file }">
                                            <div>
                                                ...
                                                <span
                                                    ...
                                                    @click="configImageAd_handleImageRemove(file,state.configImageAd.uploadImgs[scope.row.key])"
                                                >
                ...
                <div v-show="state.routeEnumMapper == 'CUSTOM_6150100'">
                    <el-form-item label="内容" :error="error.configCustomHtml.content" >
                        <div class="editor-container">
                            ...
                            <textarea ref="configCustomHtml_htmlContent_ref" placeholder="请输入代码"></textarea>
                        </div>
                    </el-form-item>
                </div>
...
<script lang="ts" setup>
    ...
    import type { DynamicRoute, DynamicRouteGroup, Tag } from '@/types';
    ...
    //查询添加前台API
    const queryAddFrontendApi = () => {
        proxy?.$axios({
            url: '/control/frontendApi/manage',
            method: 'get',
            params: {
                method : 'addFrontendApi',
            },
            ...
            if(response){
                ...
                if(result){
                    if(result.code === 200){//成功
                        const { customDynamicRouteGroupList, multiLanguageExtensionParameterMap } = result.data;
    ...
    //处理删除图片
    const configImageAd_handleImageRemove = (file: UploadFile,fileList:UploadUserFile[]) => {
    ...
    const submitForm = () => {
        ...
        proxy?.$axios({
            url: '/control/frontendApi/manage?method=addFrontendApi',
            method: 'post',
            data: formData,
            //showLoading: false,//是否显示加载图标
            loadingMask:false,// 是否显示遮罩层
