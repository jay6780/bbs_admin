...
import { computed, isReactive, isRef, type Ref } from 'vue';
...
export function processErrorInfo_number(errorInfo:Map<string,string> , error: any,callback: () => void): void {
 ...
 /**
 * 处理错误信息(Map的key为Object类型或List类型)
 * 例如：{"configImageAdList[0].name": "图片名称不能为空", "frontendApi.name": "名称不能为空"}
 * @param errorInfo 错误信息
 * @param error reactive()对象
 * @param callback 关闭弹出框时执行回调
 */
export function processErrorInfo_object(errorInfo:Map<string,string> , error: any,callback: () => void): void {
    ...
    A:for (const [key, value] of Object.entries(errorInfo) as [string, string][]){
        ...
        // 动态匹配 `listName[index].fieldName` 例如：{"configImageAdList[0].name": "图片名称不能为空"}
        const match = key.match(/^(.+)\[(\d+)\]\.(.+)$/);

        //是否为能正确映射的List类型属性
        let isListProperty = false;
        if (match) {
            const listName = match[1];     // 例如: 'configImageAdList'
            const index = parseInt(match[2]);   // 例如: 0
            const backendFieldName = match[3];  // 例如: 'name'

            let attribute = error[listName];
            if(getType(attribute[backendFieldName]) == 'map'){
                isListProperty = true;
                attribute[backendFieldName].set(index,value);
            }
        }else if(key.indexOf(".") != -1){//topicEditorTagObject.imageFormat
        ...
        if(!Object.prototype.hasOwnProperty.call(error, key) && !isListProperty){//如果属性不存在
            htmlMessage+= count+"、"+value+"<br/>";
            count ++;
        }else{
            Object.assign(error, {[key] : value});//将变量作为一个对象的key
...
export function scrollToAnchor(anchor:string) {
    if(document.querySelector("[aria-label*='前台文档']")){//前台文档列表 --> 文档
        //获取滚动窗口
        let scrollContainer:any = (document.querySelector("[aria-label*='前台文档']") as any).querySelector(".el-dialog")
        //let scrollContainer:any = document.querySelector(".el-dialog")


        //const container = position.querySelector('.documentMarkdownContent');
        // 选择器查找所有具有 id 属性的元素
        ...
        //const headingElements = container.querySelectorAll('h2[id], h3[id]');
         ...
         //获取目标锚点元素
        const targetElement =  scrollContainer.querySelector(anchor);
        if (!targetElement) {
            return;
        }

        // 确保 targetElement 是 HTMLElement
        const target = targetElement as HTMLElement;

         //文档名称的容器高度
		let document_container_height = (document.querySelector(".document-container") as any).offsetHeight;
        
        scrollContainer.scrollTo({
            top: target.offsetTop +document_container_height + 60,
            behavior: 'smooth'
        });
...
type DynamicMap<TKey, TValue> = Map<TKey, TValue>;

/**
 * 创建一个用于 v-model 的可写 computed 属性，操作任何响应式 Map 中的动态键。
 * 当 Map 中不存在该键时，getter 返回 undefined。
 * @param reactiveMap - 响应式的 Map 实例 (例如 state.commentStatusField)
 * @param key - 动态的键 (例如 comment.id 或 reply.id)
 * @returns {ComputedRef<TMapValue | undefined>} - 可双向绑定的 computed 属性
 */
export function useDynamicMapVModel<TMapValue = number>(
    reactiveMap: DynamicMap<string, TMapValue>,
    key: string
) {
    if (!isReactive(reactiveMap) && !isRef(reactiveMap)) {
        console.error("useDynamicMapVModel requires a reactive Map object.");
    }

    return computed({
        // Getter: 直接返回 Map 中的当前值 (如果不存在则为 undefined)
        get() {
            return reactiveMap.get(key);
        },
        // Setter: 将新值写入 Map
        set(newValue: TMapValue | undefined) {
            if (newValue !== undefined) {
                reactiveMap.set(key, newValue);
            }
        }
    }) as Ref<TMapValue | undefined>;
}
