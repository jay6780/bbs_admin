...
//用户扩展表单值
export interface ExtensionFormValue {
    [propName: string]: string | Array<string>;//名称
}
...
export interface MailModule{
    type: number;//类型  10：验证码
    content: string;//名称
    variableMap: Map<string,string>;//支持变量 key:变量字段  value:备注
}
//风控敏感词
export interface RiskSensitiveWords{
    id: number;//Id
    name: string;//名称
    lastModified: string;//最后修改时间
    words?: string;//敏感词
    wordsList: Array<string>;//敏感词
    version: number;//版本
}
//风控处罚
export interface Penalty{
    riskControlBlockedFeaturesList: Array<RiskControlBlockedFeatures>;//禁止的功能集合
    banDuration: number;//禁止时长
    banDurationUnit: number;//禁止时长单位  10.分钟 20.小时 30.日 40.月 50.年
    blockIP: boolean;//封禁IP
    blockIpDuration: number;//封禁IP时长
    blockIpDurationUnit: number;//封禁IP时间单位  10.分钟 20.小时 30.日 40.月 50.年
    riskControlPostFeaturesList: Array<RiskControlPostFeatures>;//禁止的功能集合
}

//风控禁止的功能
export interface RiskControlBlockedFeatures{
    name: string;//名称
    code: number;// 编号
    selected: boolean;// 选中
}
//风控执行贴子的功能
export interface RiskControlPostFeatures{
    name: string;//名称
    code: number;// 编号
    selected: boolean;// 选中
}
//处罚用户信息
export interface PenaltyUserInfo{
    id: string;//Id
    userName: string;//用户名称
    userId: string;//用户Id
    account: string;//账号
    ...
    riskControlModelId: number;//风控模型Id
    riskControlModelName: string;//风控模型名称
    penaltyParameter?: string;//风控处罚参数(处罚用户部分) Penalty类型的json格式
    penalty:  Penalty;//风控处罚
    status: number;//状态 0.无  10.生效中(此状态不存储，由程序运行时设置) 20.已结束(此状态不存储，由程序运行时设置) 30.已解除封禁
    unblockTime: string;//解除封禁时间
    staffAccount: string;//解除封禁员工账号
    sensitiveWord: string;//触发敏感词
    sensitiveWordList: Array<string>;//触发敏感词集合
}
//处罚贴子信息
export interface PenaltyPostInfo{
    id: string;//Id
    userName: string;//用户名称
    userId: string;//用户Id
    account: string;//账号
...
export interface VoteTheme{
    ...
    sourceParameterTitle: string;//来源参数标题(话题标题、问题标题)
...
export interface FrontendApi{
    ...
    requiresLogin: boolean;//访问需要登录
    routeEnumMapper: string;//映射路由枚举值
    routeEnumName: string;//映射路由枚举名称
    type?: number;//类型 1.默认  2.自定义    由Controller类上注解指定映射路径的为默认；由数据库配置映射路径的为自定义
}
...
export interface DynamicRouteGroup {
    groupEnum: string;//组枚举值
    name: string;//名称
    code: string;//编号
    dynamicRouteList:Array<DynamicRoute>//动态路由集合
}
/**
 * 动态路由
 */
 ...
 * 图片广告参数配置
 */
export interface ConfigImageAd {
    id: string;//Id
    name: string;//图片名称
    link: string;//图片链接
    imagePath: string;//图片路径
    multiLanguageExtensionField: Map<string,string>;
}
